...
// config.js). Каждый запрос тегируется текущей ступенью (stage_rps), а
// пороги ниже объявлены и для всего прогона, и отдельно для каждой ступени —
// это позволяет handleSummary() напечатать ступень, на которой сервис
// впервые перестал укладываться в пороги (см. lib/stageThresholds.js, lib/maxRpsSummary.js).
const thresholdRules = {
    http_req_failed: ['rate<0.01'],
    http_req_duration: ['p(99)<500'],
};

const RESULTS_FILE = 'results/notes-get-by-id.md';
