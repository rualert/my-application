...
- `README.md` — run instructions (local + Docker + Docker Compose/ELK + tracing).
...
- `src/MyApplication/MyApplication.Api/` — ASP.NET Core Web API project (Presentation layer). Entry point: `Program.cs`, which wires up DI (registering `Application` layer services), Serilog, and OpenTelemetry tracing. Uses attribute-routed controllers (`AddControllers()` / `MapControllers()`), not minimal-API endpoints — all external call handlers live in `Controllers/`, calling into `MyApplication.Application` services/interfaces rather than containing business logic themselves. Depends on `MyApplication.Application`.
