...
Оба сервиса (`api`, `web-ui`) подключены к репозиторию `rualert/my-application`, ветка `master`: слияние PR в `master` само запускает сборку и выкатку — вручную ничего делать не нужно. Чтобы не пересобирать всё на каждый коммит, у сервисов заданы watch paths:

| Сервис | Watch paths |
|---|---|
| `api` | `/src/MyApplication/**`, `/ci/Dockerfile` |
| `web-ui` | `/src/MyApplication.Web.UI/**`, `/ci/Dockerfile.web-ui`, `/ci/nginx.conf.template` |

Слияние, которое не затрагивает эти пути (например, правка `docs/`), деплой не запускает. Если правка внешне «не про сервис», но влияет на его сборку (новый файл, который `COPY`-ится в Dockerfile, а лежит вне этих путей) — добавьте путь в watch paths сервиса.

«Wait for CI» на стороне Railway **выключен** сознательно: гарантию «в `master` попадает только зелёное» даёт branch protection на GitHub — `master` требует PR и прохождения проверок `backend` и `web-ui` (`tests.yml`) при актуальной ветке относительно `master`. Администратор репозитория может обойти защиту прямым push — такой коммит уйдёт в прод без тестов.
...
Ручной передеплой сервиса из локального кода (из корня репозитория) по-прежнему работает — например, чтобы выкатить незамерженную правку. Учтите, что следующий автодеплой из `master` перезапишет такую выкатку:
